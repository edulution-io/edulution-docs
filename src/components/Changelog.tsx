...
const Tag: React.FC<{ tag: string }> = ({ tag }) => {
  const colors: Record<string, string> = {
    'edulution-ui': 'bg-[rgba(136,216,64,0.15)] text-[#88d840] border-[#88d840]/30',
  ...
  return (
    <span className={`inline-block px-2 py-0.5 text-[0.625rem] font-medium rounded border uppercase tracking-wide ${colorClass}`}>
      {tag}
    </span>
  );
};
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 lg:flex lg:px-8">
      <div className="lg:ml-48 lg:flex lg:w-full lg:justify-start lg:pl-16">
        <div className={`mx-auto max-w-3xl lg:mx-0 lg:w-full lg:max-w-3xl ${className}`}>
          {children}
        </div>
      </div>
    </div>
...
// Article
export const ChangelogItem: React.FC<{ entry: ChangelogEntry; index: number }> = ({ entry, index }) => {
  ...
  const id = `v${entry.version || index}`;
  ...
  return (
    ...
    >
      <div ref={heightRef}>
        ...
        <ContentWrapper className="changelog-typography">
          <div className="flex items-center gap-3 mb-2">
            <h2 className="font-display text-2xl font-bold leading-8 text-white">
              {entry.title}
            </h2>
            {entry.version && (
              <span className="inline-flex items-center px-3 py-1 text-xs font-semibold text-[#88d840] bg-[#88d840]/10 rounded-full border border-[#88d840]/20">
                v{entry.version}
              </span>
            )}
          </div>
          ...
          {/* Render content blocks in flow */}
          {entry.content && entry.content.length > 0 ? (
            <div className="space-y-6">
              {entry.content.map((block, idx) => {
                if (block.type === 'image') {
                  return (
                    <div key={idx} className="relative overflow-hidden rounded-2xl bg-gray-900/50 group cursor-pointer transition-all duration-300 hover:ring-2 hover:ring-[#88d840]/50">
                      <img
                        src={block.url}
                        alt={block.alt}
                        className="w-full h-auto max-w-full transition-transform duration-300 group-hover:scale-[1.02]"
                        loading="lazy"
                        style={{ display: 'block' }}
                      />
                ...
                if (block.type === 'improvements') {
                  return (
                    <div key={idx} className="p-6 rounded-xl bg-gradient-to-br from-gray-900/50 to-gray-800/30 border border-gray-800/50">
                      <h3 className="flex items-center gap-3 text-lg font-semibold leading-6 text-white mb-6">
                        <div className="p-2 rounded-lg bg-[#88d840]/10">
                          <SparkleIcon />
                        </div>
                        {block.title}
                      </h3>
                      <ul className="list-none pl-0 m-0 space-y-3">
                        {block.items.map((item, itemIdx) => (
                          <li key={itemIdx} className="relative pl-6 text-sm leading-7 text-gray-300 hover:text-gray-100 transition-colors">
                            <span className="absolute left-0 top-[0.6rem] w-1.5 h-1.5 rounded-full bg-[#88d840]"></span>
                            {renderMarkdown(item)}
                          </li>
                        ))}
                      </ul>
                ...
                if (block.type === 'link') {
                  return (
                    <div key={idx}>
                      <a
                        href={block.url}
                        className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-[#88d840] bg-[#88d840]/5 rounded-lg border border-[#88d840]/20 hover:bg-[#88d840]/10 hover:border-[#88d840]/40 transition-all duration-200"
                        target="_blank"
            ...
            <>
              {/* Legacy rendering */}
              {entry.image && (
                <div className="relative overflow-hidden rounded-2xl bg-gray-900/50 mb-8 group cursor-pointer transition-all duration-300 hover:ring-2 hover:ring-[#88d840]/50">
                  <img
                    src={entry.image}
                    alt={entry.title}
              ...
              {entry.images && entry.images.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                  {entry.images.map((img, idx) => (
                    <div key={idx} className="relative overflow-hidden rounded-xl bg-gray-900/50 group cursor-pointer transition-all duration-300 hover:ring-2 hover:ring-[#88d840]/50">
                      <img
                        src={img}
                ...
                <div className="mt-6 p-6 rounded-xl bg-gradient-to-br from-gray-900/50 to-gray-800/30 border border-gray-800/50">
                  ...
                  <ul className="list-none pl-0 m-0 space-y-3">
                    {entry.improvements.map((improvement, idx) => (
                      <li key={idx} className="relative pl-6 text-sm leading-7 text-gray-300 hover:text-gray-100 transition-colors">
                        <span className="absolute left-0 top-[0.6rem] w-1.5 h-1.5 rounded-full bg-[#88d840]"></span>
...
export const Changelog: React.FC<ChangelogProps> = ({ entries }) => {
  ...
  return (
    <div id="tw-scope" className="relative flex-auto">
      {/* Suche und Filter */}
      <div className="sticky top-0 z-50 border-b border-gray-800/50 backdrop-blur-xl py-6" style={{ background: 'var(--ifm-background-color)' }}>
        <ContentWrapper>
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 border border-gray-700/50 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-[#88d840] focus:ring-2 focus:ring-[#88d840]/20 transition-all"
            ...
            <div className="flex gap-2 flex-wrap">
              <button
                ...
                className={`px-5 py-3 text-sm font-semibold rounded-xl transition-all duration-200 ${
                  selectedTag === 'all'
                    ? 'bg-[#88d840] text-black shadow-lg shadow-[#88d840]/20'
                    : 'text-gray-300 border border-gray-700/50 hover:border-[#88d840]/50 hover:bg-[#88d840]/5'
                }`}
      ...
      <main className="space-y-16 py-12 sm:space-y-20 sm:py-16">
        {filteredEntries.length > 0 ? (
          filteredEntries.map((entry, index) => (
            <ChangelogItem key={`${entry.version}-${entry.date}-${index}`} entry={entry} index={index} />
          ))
        ) : (
          <ContentWrapper>
            <div className="text-center py-20">
              ...
              <button
                ...
                className="mt-4 px-4 py-2 bg-[#88d840] text-black rounded-lg font-medium hover:bg-[#96dc55] transition-colors"
              >
                Filter zurücksetzen
              </button>
            </div>
