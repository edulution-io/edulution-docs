...
Betreiben Sie Moodle bereits selbst, tragen Sie unter **Einstellungen** → **Lernmanagement** lediglich die **URL** Ihrer Moodle-Instanz ein. Die Installation und Konfiguration von Moodle beschreibt der [Schnellstart](/docs/edulution-plattform/apps/mit-einrichtung/lernmanagement/installation/schnellstart) im Bereich edulution Moodle.
...
1. **Keycloak-Verbindung** herstellen und die [Synchronisation](/docs/edulution-plattform/apps/mit-einrichtung/lernmanagement/konfiguration/synchronisation) aktivieren, damit Benutzer, Kurse und Einschreibungen entstehen.
2. **Cookie Auth (SSO)** aktivieren: *Site-Administration → Plugins → Edulution → Cookie Auth (SSO)*. Für edulution sind die Standardwerte richtig – Cookie-Name `authToken`, Benutzer-Claim `preferred_username`. Alle Einstellungen beschreibt die Seite [Cookie Auth (SSO)](/docs/edulution-plattform/apps/mit-einrichtung/lernmanagement/konfiguration/cookie-auth).
...
Wie bei jeder App legen Sie unter **Einstellungen** → **Lernmanagement** fest, wo die App erscheint (Seitenleiste, Menüleiste, App-Launcher) und welche Nutzergruppen sie sehen. Details dazu unter [Einstellungen](../../../konfiguration/einstellungen.md).
...
| Anmeldung schlägt für **einzelne Benutzer** fehl | Das Konto existiert noch nicht in Moodle. Führen Sie die [Synchronisation](/docs/edulution-plattform/apps/mit-einrichtung/lernmanagement/konfiguration/synchronisation) aus. |
...
| Kurse fehlen oder heißen **unerwartet** | Die Kurse entstehen aus den Gruppennamen. Siehe [Gruppen-Namensschemas](/docs/edulution-plattform/apps/mit-einrichtung/lernmanagement/konfiguration/namensschemas). |

Bleibt das Problem bestehen, hilft die Testseite des Plugins weiter, die Cookie, Token und Konfiguration anzeigt – beschrieben unter [Cookie Auth (SSO) → Testen](/docs/edulution-plattform/apps/mit-einrichtung/lernmanagement/konfiguration/cookie-auth#testen).

## Siehe auch

- [edulution Moodle – Übersicht](/docs/edulution-plattform/apps/mit-einrichtung/lernmanagement/konfiguration/) – Plugin, Synchronisation und Kursstruktur
- [Cookie Auth (SSO)](/docs/edulution-plattform/apps/mit-einrichtung/lernmanagement/konfiguration/cookie-auth) – alle Einstellungen der automatischen Anmeldung
- [Synchronisation](/docs/edulution-plattform/apps/mit-einrichtung/lernmanagement/konfiguration/synchronisation) – Benutzer, Kurse und Einschreibungen abgleichen
- [App-Store & Anwendungen](../../direkt-einsatzbereit/app-store.md) – Apps hinzufügen und verwalten
- [Einstellungen (Settings)](../../../konfiguration/einstellungen.md) – Container, Nutzergruppen und Anzeigeorte
