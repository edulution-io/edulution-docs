...
const CodeBlock: React.FC<CodeBlockProps> = ({ variants, title, description }) => {
  ...
  return (
    <div id="tw-scope" className="my-6 rounded-lg border border-gray-300 dark:border-[#2a2a2a] overflow-hidden">
      {/* Header */}
      <div className="bg-gray-100 dark:bg-[#1a1a1a] px-4 py-3 flex items-center justify-between border-b border-gray-300 dark:border-[#2a2a2a]">
        ...
        <div className="flex items-center gap-3">
              ...
              {availablePlatforms.map(platform => (
                <button
                  ...
                  style={selectedPlatform === platform ? {
                    background: 'linear-gradient(45deg, #88D840, #0081C6)'
                  } : undefined}
                >
                  {platform}
                </button>
          ...
          <button
            ...
            onMouseLeave={(e) => {
              e.currentTarget.style.background = 'linear-gradient(45deg, #88D840, #0081C6)';
            }}
            title="Kopieren"
          >
      ...
      {processedOutput && (
        <div className="border-t border-gray-300 dark:border-[#2a2a2a]">
          ...
          <div className="bg-gray-50 dark:bg-[#0f0f0f]">
            <pre className="p-4 overflow-x-auto text-sm">
              <code className="text-green-600 dark:text-[#88d840]">{processedOutput}</code>
            </pre>
          </div>
        </div>
      )}
