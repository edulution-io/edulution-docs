...
const FeatureTag: React.FC<{ type: string }> = ({ type }) => {
    ...
    return (
        <span className={`inline-block px-2 py-0.5 text-[0.625rem] font-medium rounded border ${colorClass}`}>
      {label}
    </span>
    );
};
                    ...
                    {entry.content && entry.content.length > 0 ? (
                        <div className="space-y-6">
                            {entry.content.map((block, idx) => {
                                if (block.type === 'image') {
                                    return (
                                        <div key={idx} className="relative overflow-hidden rounded-2xl bg-gray-900/50 mb-8 group cursor-pointer transition-all duration-300 hover:ring-2 hover:ring-[#88d840]/50">
                                            <img
                                                src={block.url}
                                                alt={block.alt}
                        ...
                        <span className="text-base font-semibold text-white">
                          {renderMarkdown(block.text)}
                                ...
                                if (block.type === 'improvements') {
                                    return (
                                        <div key={idx} className="mt-6 p-6 rounded-xl bg-gradient-to-br from-gray-900/50 to-gray-800/30 border border-gray-800/50">
