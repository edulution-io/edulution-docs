...
2.1 SSL-Zertifikat Verwaltung
******************************

.. index:: SSL-Verwaltung, Zertifikat-Erneuerung, Traefik

2.1.1 Zertifikat-Status prüfen
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Um den aktuellen Status des SSL-Zertifikats zu überprüfen::

    # Zertifikat-Informationen anzeigen
    docker exec -it traefik sh -c "cat /ssl/cert.cert | openssl x509 -text -noout"
    
    # Ablaufdatum prüfen
    docker exec -it traefik sh -c "cat /ssl/cert.cert | openssl x509 -enddate -noout"

2.1.2 Zertifikat manuell erneuern
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

**Selbstsigniertes Zertifikat erneuern:**

1. Stoppen Sie den Traefik-Container:
...
2. Generieren Sie ein neues selbstsigniertes Zertifikat:

   .. code-block:: bash

      openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
          -keyout data/traefik/ssl/cert.key \
          -out data/traefik/ssl/cert.cert

3. Starten Sie Traefik neu:

   .. code-block:: bash

      docker compose start traefik

**Let's Encrypt Zertifikat manuell erneuern:**

Let's Encrypt Zertifikate werden normalerweise automatisch erneuert. Falls eine manuelle Erneuerung nötig ist:

.. code-block:: bash

   # Traefik Container neustarten (löst Erneuerung aus)
   cd |docker-path|
   docker compose restart traefik

**Eigenes Zertifikat austauschen:**

1. Kopieren Sie die neuen Zertifikatsdateien:

   .. code-block:: bash

      cp /pfad/zum/neuen/zertifikat.crt /srv/docker/edulution-ui/data/traefik/ssl/cert.cert
      cp /pfad/zum/neuen/schluessel.key /srv/docker/edulution-ui/data/traefik/ssl/cert.key

2. Stellen Sie die korrekten Berechtigungen sicher:

   .. code-block:: bash

      chmod 600 /srv/docker/edulution-ui/data/traefik/ssl/cert.key
...
2.1.3 Von selbstsigniert zu Let's Encrypt wechseln
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Falls Sie nachträglich von einem selbstsignierten Zertifikat zu Let's Encrypt wechseln möchten:

1. Stellen Sie sicher, dass Ihre Domain öffentlich erreichbar ist
2. Bearbeiten Sie die Traefik-Konfiguration:
...
3. Fügen Sie die Let's Encrypt Konfiguration hinzu (siehe Installationsdokumentation)
...
2.1.4 SSL-Probleme diagnostizieren
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. code-block:: bash
    ...
    # Von extern
    openssl s_client -connect ui.musterschule.de:443 -servername ui.musterschule.de
    
    # Zertifikatskette prüfen
    curl -vI https://ui.musterschule.de

**Let's Encrypt Debug-Modus:**

Für detaillierte Let's Encrypt Fehler, aktivieren Sie den Debug-Modus in der Traefik-Konfiguration.

2.2 Backup und Wiederherstellung
*********************************
